...
func (h *Handler) handleToolsList() *toolsListResult {
	return &toolsListResult{
		Tools: []toolDef{{
			...
			Description: "Search the configured codebase (grep-like). Returns file path, line range, and snippet. Deterministic, read-only.",
			InputSchema: inputSchema{
				Type: "object",
				Properties: map[string]propDef{
					"query":     {Type: "string", Description: "Search query (required)"},
					"language":  {Type: "string", Description: "Optional language filter (e.g. go, py)"},
					"path_hint": {Type: "string", Description: "Optional path substring to narrow directories"},
					"limit":     {Type: "number", Description: "Max matches (default 10, max 20)"},
				},
...
func (h *Handler) handleToolsCall(params json.RawMessage) *toolsCallResult {
	...
	if p.Name != "search_internal_codebase" {
		return &toolsCallResult{
